...
public:
    ...
    void execute(bool verbose = false)
    {
        ...
        for (size_t i = 0; i < n_count; ++i) {
            ...
            std::string text;
            FileSystem::readFile(filename, text);
...
{
    if (!FileSystem::exists("multi")) {
        FileSystem::exec("mpic++ multi.cpp -I include/ -o multi");
    }

    std::vector<int> steps { 5000, 10000, 25000, 50000 };
    std::vector<int> threads { 2, 3, 4, 5 };

    SetupRecord setup = { steps, threads };
    Measurement measurement(setup);

    measurement.execute_mean(10, true);
