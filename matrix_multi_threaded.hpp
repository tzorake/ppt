...
//! Matrix_M implementation, with a series of linear algebra functions
...
public:
    ...
    Matrix_M(int dimension) 
    {
        Matrix_M(dimension, dimension);
    }

    //! Initializes a matrix with a predetermined number of rows and columns
    //! \param rows number of rows in the matrix
    //! \param cols number of columns in the matrix
    Matrix_M(int rows, int cols) : mRows(rows), mCols(cols), mData(rows* cols) 
    {
    ...
    Matrix_M(int rows, int cols, const vector<T>& data) : mRows(rows), mCols(cols) 
    {
        if (data.size() != rows * cols)
        {
            throw invalid_argument("Matrix_M dimension incompatible with its initializing vector.");
        }
        mData = data;
    }

    template<int N>
    Matrix_M(int rows, int cols, T(&data)[N]) 
    {
        if (N != rows * cols)
        {
            throw invalid_argument("Matrix_M dimension incompatible with its initializing vector.");
    ...
    friend Matrix_M operator+(const Matrix_M& m, double value) 
    {
        Matrix_M result(m.mRows, m.mCols);
    ...
    //! \param value scalar to be added to the matrix
    //! \return the result of the scalar addition of <code>m</code> and <code>value</code>
    friend Matrix_M operator+(double value, const Matrix_M& m) 
    {
        return m + value;
    }
    ...
    Matrix_M operator+=(double value) 
    {
    ...
    //region Matrix_M operators

    //! Matrix_M addition operation
    //! \param b another matrix
    //! \return Result of the addition of both matrices
    Matrix_M operator+(const Matrix_M& b) 
    {
    ...
    //! Matrix_M subtraction operation
    //! \param b another matrix
    ...
    //! Matrix_M multiplication operation
    //! \param b another matrix
    //! \return Result of the multiplication of both matrices
    Matrix_M operator*(const Matrix_M& b) const 
    {
        if (mCols != b.mRows)
        {
        ...
        Matrix_M result = zeros(mRows, b.mCols);
    ...
    Matrix_M& operator+=(const Matrix_M& other) 
    {
    ...
    Matrix_M& operator*=(const Matrix_M& other) {
        ...
        Matrix_M result(mRows, other.mCols);
    ...
    Matrix_M<int> operator==(const T& value) 
    {
        Matrix_M<int> result(mRows, mCols);
    ...
    bool operator==(const Matrix_M& other) 
    {
    ...
    Matrix_M operator!=(const double& value) 
    {
    ...
    //! Matrix_M negative operation
    //! \return The negative of the current matrix
    Matrix_M operator-() 
    {
        Matrix_M result(this->mRows, this->mCols);
    ...
    static Matrix_M fill(int rows, int cols, double value) 
    {
        Matrix_M result(rows, cols, vector<T>(rows * cols, value));
        return result;
    }
    ...
    static Matrix_M zeros(int rows, int cols) 
    {
    ...
    Matrix_M submatrix(int row, int column) const 
    {
        Matrix_M result(mRows - 1, mCols - 1);
    ...
    Matrix_M inverse() const 
    {
        ...
        if (det == 0)
        {
            throw runtime_error("Matrix_M is singular");
        }

        Matrix_M adj = adjugate();
    ...
    Matrix_M transpose() const 
    {
        Matrix_M result(mCols, mRows);
    ...
    friend ostream& operator<<(ostream& os, const Matrix_M& matrix) 
    {
    ...
    Matrix_M<T> apply(function<T(T)> f) 
    {
        Matrix_M<T> result(mRows, mCols, vector<T>(mRows * mCols, 0));
...
typedef Matrix_M<double> MatrixD_M;
typedef Matrix_M<int> MatrixI_M;
